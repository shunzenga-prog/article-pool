...
│   ├── gen_cover.py           # 封面图生成（1200×675，auto/geometric）
...
│       ├── cover-agent.md     # 封面生成（强制 auto 模式）
...
```
写作 → 审阅 Agent（HTML 结构扫描）→ 封面 Agent（auto 模式）→ 插图 Agent → 发布 Agent
```
...
| cover-agent | Agent/Codex 本地背景图优先，兼容 auto 旧来源，验证 >100KB |
...
python scripts/gen_cover.py --title "标题" --background-image cover-bg.png --output cover.png

# 不要用 --mode geometric（除非封面生成失败需要兜底）
```
