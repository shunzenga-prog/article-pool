...
创作流程由 Codex 直接按 `skills/` 中的 SKILL.md 执行。技能负责"做什么"，脚本负责"执行"，审阅脚本负责"质量门禁"。
...
| 封面 | Agent/image_gen 直接生成最终 1200×675 PNG；无生图能力时才用 legacy `gen_cover.py`，>100KB | CSDN 用文章首图自动提取 |
| 发布 | `python scripts/publish_html.py` 见 ✅ + draft ID | `python scripts/publish_csdn.py` 见 ✅ + articleId |

### 创作流程

```
用户选题 → 选平台（公众号/CSDN）→ Codex 读取对应 SKILL.md →
...
由 `cover-agent`（`agents/article-pool/cover-agent.md`）自动执行。

**Codex / GPT Image 硬约束：** 当前 Agent 具备 GPT Image / image_gen 生图能力时，封面必须由 Agent 根据文章语义直接生成最终 1200×675 PNG。禁止先生成背景图再调用 `gen_cover.py --background-image` 二次处理。`gen_cover.py` 只在当前环境没有生图能力、用户明确要求真实图库/事实图片，或旧链路兜底时使用；`geometric` 不能作为成功结果。

旧兜底命令只写在 `skills/cover-gen/SKILL.md` 的 legacy fallback 部分；默认创作流程不得复制执行 `gen_cover.py`。
...
| # | 检查项 | 验证方法 |
|---|--------|----------|
| 1 | HTML 文章已生成 | 文件存在于 `/Users/mulin/workspace/公众号/文章/{YYYY}年{MM}月/` |
...
```
公众号：/Users/mulin/workspace/公众号/文章/{YYYY}年{MM}月/{MMDD}-{标题}.html（封面同名 .png）
CSDN：  /Users/mulin/workspace/公众号/文章/{YYYY}年{MM}月/{MMDD}-{标题}.md
```
