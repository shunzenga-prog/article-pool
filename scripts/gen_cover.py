# -*- coding: utf-8 -*-
"""Professional cover image generator for WeChat articles — multi-theme edition.

8 themes selected by title hash. Load theme data from gen_cover_themes.json.
Usage:
    python3 gen_cover.py --title "title" --output cover.png
    python3 gen_cover.py --list-themes
"""

from PIL import Image, ImageDraw, ImageFont
import os, math, random, argparse, hashlib, json
...
THEMES_FILE = Path(__file__).resolve().parent / 'gen_cover_themes.json'
...
def _tuple(c):
    """Parse color from JSON list [r,g,b] to tuple."""
    return tuple(c)

def build_background(img, draw, px, theme):
...
def generate_cover(title, subtitle, tag, date_str, reading_info, footer, output_path, theme_name=None):
    themes = load_themes()
    ...
    px = img.load()
    build_background(img, draw, px, theme)

    ACCENT = _tuple(theme["accent"])
    LIGHT, MID, DIM = (0xE8,0xE8,0xF0), (0x94,0x94,0xB8), (0x64,0x64,0x88)
    ...
    draw.text((W-280,80), day_num, fill=_tuple(theme["watermark_color"]), font=fb)

    ft = ImageFont.truetype(FLAT, 11)
    bt = theme["name"]
    bb = draw.textbbox((0,0), bt, font=ft)
    bw = bb[2]-bb[0]+12
    draw.rectangle([W-bw-20,592,W-20,610], fill=_tuple(theme["watermark_color"]))
    draw.text((W-bw-14,593), bt, fill=DIM, font=ft)

    draw_text(draw, (LEFT,605), footer, DIM, f16c, f14l)
    img.save(output_path)
    print(f'OK: {output_path} ({os.path.getsize(output_path)/1024:.1f} KB) theme: {theme["name"]}')
...
def list_themes():
    ...
    for k, t in themes.items():
        c = t["accent"]
        print(f"  {k:12s} - {t['name']}  (accent #{c[0]:02x}{c[1]:02x}{c[2]:02x})")

def main():
    today = datetime.now()
    dd = today.strftime('%Y / %m / %d')
    p = argparse.ArgumentParser(description='Generate 1200x675 WeChat cover (multi-theme).')
    ...
    themes = load_themes()
    p.add_argument('--theme', choices=list(themes.keys()), help='Force a theme.')
    p.add_argument('--list-themes', action='store_true', help='List themes.')
    args = p.parse_args()
    if args.list_themes:
        list_themes()
        return
    if not args.title or not args.output:
        p.error("--title and --output are required")
    generate_cover(args.title, args.subtitle, args.tag, args.date,
                   args.reading_time, args.footer, args.output, args.theme)
