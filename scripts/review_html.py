...
def visible_text(html: str) -> str:
    """Return reader-visible text for wording checks."""
...
def review(html_path: str, tutorial: bool = False, title: str | None = None) -> dict:
    ...
    ai_formulaic_matches = []
    for pattern in ai_formulaic_patterns:
        ai_formulaic_matches.extend(re.findall(pattern, text, re.IGNORECASE))
    ...
    if ai_formulaic_matches:
        hard_failures.append(f"H5: AI 套话/公式化表达 {len(ai_formulaic_matches)} 处")
    if broadcast_matches:
        hard_failures.append(f"H5b: 新闻播报腔表达 {len(broadcast_matches)} 处")
    ...
    return {
        ...
        "hard_checks": {
            ...
            "h5_ai_formulaic_phrases": {
                "status": "FAIL" if ai_formulaic_matches else "PASS",
                "count": len(ai_formulaic_matches),
                "detail": ai_formulaic_matches[:8] if ai_formulaic_matches else "无 AI 套话/公式化表达",
            },
            "h5_broadcast_tone": {
