...
3. **时效** → <24h 可用，>48h 转分析
4. **风格规划** → ⭐ 定一张「风格卡」（基调+配色+强调+节奏），后续所有写作和排版都以此为指南（详见下方「风格规划」）
...
13. **发布** → ①上传图片到微信CDN生成 `_publish.html` → ②推送到草稿箱 → ③题目入库
...
**不要先选模板再填内容——先感受选题的"气质"，再让视觉跟着内容走。**

### 风格卡格式

用自然语言定一张风格卡（不是表格、不是填空题），3-5 句即可：

```
风格卡：
- 基调：[理性冷静 / 温暖亲切 / 犀利直接 / 轻松幽默]
- 主色+辅色：[为什么选这个色？跟文章情绪什么关系？]
- 强调方式：[数字怎么突出？重点怎么标记？代码块什么风格？]
- 节奏：[标题间隔？图文比例？是否需要引用框？]
```
...
| 文章类型 | 典型基调 | 常见配色思路 |
|---------|---------|------------|
| 技术教程 | 干净理性 | 蓝灰为主，绿色标注"成功/正确"，红色标注"注意/警告" |
| 深度分析 | 沉稳克制 | 深色为主，金色/暖色标注关键结论，大量留白 |
| 热点快评 | 犀利直接 | 高对比，红/黑为主，短句，节奏快 |
| 工具推荐 | 亲切实用 | 明亮但不花哨，品牌色系，卡片式展示 |
| 个人故事 | 温暖真实 | 暖色系，微信默认风格即可，不要过度设计 |
...
风格卡（本期教程：AI数据分析实战）：
- 基调：技术实操，干净理性，不花哨
- 主色 #1E88E5（冷静蓝），辅色 #27AE60（数据绿），强调 #E74C3C（警示红）
- 强调：关键数字用加粗+绿色，代码块用暗色终端风格（#1e1e2e 背景），引用用左边框蓝色
- 节奏：每 300 字一个标题，图表紧跟文字说明，段落不超过 5 行
```
...
| 超过 4 种颜色混在一起 | 主色+辅色+强调色，其余用灰度 |
| 大段渐变背景、阴影、圆角（公众号会剥离） | 扁平、干净，用左边框或底部线做分隔 |
| 每个段落都换一种颜色 | 全文统一配色，仅在不同功能区域（引用/思考/数据）用色区分 |
| 代码块用亮色背景（白底代码块在手机端刺眼） | 代码块统一暗底浅字 |

**检查方法：** 生成 HTML 后，退一步看——如果觉得"花"，就减一种颜色；如果觉得"平"，就加一处强调。
