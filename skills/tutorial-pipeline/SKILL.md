...
| ❌ 错误做法 | ✅ 正确做法 |
|------------|------------|
| "AI 用 IQR 法检测了异常值" | 展示 IQR 计算代码 + 解释 Q1/Q3 含义 + 说明为什么 1.5 倍 |
| "AI 自动选择了 utf-8-sig 编码" | 解释 BOM 头是什么、Excel 导出的坑、为什么不用 utf-8 |
| "AI 生成了 5 张图表" | 展示一张图表的完整生成代码，解释每个 matplotlib API 的作用 |
| 只列函数名 | 展示 `main()` 函数代码，解释为什么设计成 5 步流水线 |

**每步文字的三要素：**
1. **代码片段** - 实际的关键代码（不是伪代码）
...
- ❌ 用文字代码块模拟终端输出代替真实截图
- ❌ 只描述结果不展示代码——读者学不到东西，教程沦为 AI 炫耀秀
- ❌ 代码只列函数名不做解释——读者不知道每行在干什么、为什么这样写
...
□ 代码解释完整性 - 每个核心步骤是否展示了关键代码 + 解释了设计思路？
